...
agenda-day   The day in the agenda where this is listed"
  ...
  (let* ((lines (org-split-string (buffer-string) "\n"))
	...
	(if (not (get-text-property 0 'org-category line)) (throw 'next nil))
        (let* ((props (org-fix-agenda-info (text-properties-at 0 line)))
               (category (plist-get props 'org-category))
               (events (assoc-default category cases 'string= '())))
          (setq cases (cons (cons category (append events (list props))) cases)))))
